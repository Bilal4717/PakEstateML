...
function onClickedEstimatePrice() {
    ...
    $.post(url, {
        ...
        estPrice.innerHTML = "Estimated Price: RS " + data.estimated_price.toLocaleString() ;
        estPrice.style.color = "var(--accent-color)"; 
    }).fail(function(response) {
        console.log("CSP or Server Error:", response);
        
        if (response.status === 400 && response.responseJSON) {
            
            estPrice.innerHTML = "<h3 style='color: #ff4d4d; font-size: 1rem;'>" + 
                                 response.responseJSON.error + "</h3>";
        } else {
            estPrice.innerHTML = "<h3 style='color: #ff4d4d;'>Server Error. Please try again.</h3>";
        }
    });
}
